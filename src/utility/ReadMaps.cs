...
namespace Landis.Extension.Succession.NECN
{
    ...
    public static class ReadMaps
    {
        ...
        //---------------------------------------------------------------------
        // W.Hotta (2022.05.03)
        public static void ReadProbEstAdjustMap(string path)
        {
            IInputRaster<DoublePixel> map = MakeDoubleMap(path);
            ...
            using (map)
            {
                ...
                foreach (Site site in PlugIn.ModelCore.Landscape.AllSites)
                {
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 1.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "ProbEst Adjustment value {0} is not between {1:0.0} and {2:0.0}.  Site_Row={3:0}, Site_Column={4:0}",
                                                          mapValue, 0.0, 1.0, site.Location.Row, site.Location.Column);
                        SiteVars.ProbEstAdjust[site] = mapValue;
                    }
                }
            }
        }
        //---------------------------------------------------------------------
        // W.Hotta (2022.05.07)
        public static void ReadSlopeAngleMap(string path)
        {
            IInputRaster<DoublePixel> map = MakeDoubleMap(path);
            ...
            using (map)
            {
                ...
                foreach (Site site in PlugIn.ModelCore.Landscape.AllSites)
                {
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 90.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "Slope Angle value {0} is not between {1:0.0} and {2:0.0}.  Site_Row={3:0}, Site_Column={4:0}",
                                                          mapValue, 0.0, 90.0, site.Location.Row, site.Location.Column);
                        SiteVars.SlopeAngle[site] = mapValue;
                    }
                }
            }
        }
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 90.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "Slope Angle value {0} is not between {1:0.0} and {2:0.0}.  Site_Row={3:0}, Site_Column={4:0}",
                                                          mapValue, 0.0, 90.0, site.Location.Row, site.Location.Column);
                        SiteVars.SoilMoisture[site] = mapValue;
                    }
                }
            }
        }
